...
export default function ThemeToggle({ isDarkMode, onToggle, variant = 'catalog' }) {
  return (
    <button
      ...
      onClick={(e) => {
        onToggle();
        e.currentTarget.blur(); // Evita que se quede enfocado en móviles/al hacer click
      }}
      className={`p-2.5 rounded-full transition-colors duration-200 focus:outline-none focus-visible:ring-2 ${VARIANT_STYLES[variant] ?? VARIANT_STYLES.catalog}`}
