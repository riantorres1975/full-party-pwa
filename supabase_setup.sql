...
-- ───────────────────────────────────────────────────────────────────────────
-- 4.6 TABLA PUSH_SUBSCRIPTIONS (Web Push para notificar clientes)
-- ───────────────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_push_sub_folio ON public.push_subscriptions(folio);

ALTER TABLE public.push_subscriptions ENABLE ROW LEVEL SECURITY;

-- Cualquier persona puede suscribirse (insertar su push subscription)
DROP POLICY IF EXISTS "Public puede insertar push subscriptions" ON public.push_subscriptions;
CREATE POLICY "Public puede insertar push subscriptions"
  ON public.push_subscriptions
  FOR INSERT
  WITH CHECK (true);

-- Solo admins pueden leer (la Edge Function usa service_role key, no RLS)
DROP POLICY IF EXISTS "Solo admins pueden leer push subscriptions" ON public.push_subscriptions;
CREATE POLICY "Solo admins pueden leer push subscriptions"
  ON public.push_subscriptions
  FOR SELECT
  USING (auth.uid() IN (SELECT user_id FROM public.admins));

-- Solo admins pueden eliminar (cleanup de endpoints expirados)
DROP POLICY IF EXISTS "Solo admins pueden eliminar push subscriptions" ON public.push_subscriptions;
CREATE POLICY "Solo admins pueden eliminar push subscriptions"
  ON public.push_subscriptions
  FOR DELETE
  USING (auth.uid() IN (SELECT user_id FROM public.admins));
